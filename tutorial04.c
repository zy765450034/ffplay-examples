...
typedef struct VideoState {
  ...
  int             videoStream,     //AVMEDIA_TYPE_VIDEO
                  audioStream;     //AVMEDIA_TYPE_AUDIO
  AVStream        *audio_st;       //音频流
  AVCodecContext  *audio_ctx;      //编解码器
  PacketQueue     audioq;          //音频列表
  uint8_t         audio_buf[(AVCODEC_MAX_AUDIO_FRAME_SIZE * 3) / 2];//音频缓存
  unsigned int    audio_buf_size;  //音频缓存大小sizeof(is->audio_buf)
  unsigned int    audio_buf_index; //读取音频流缓存位
  AVFrame         audio_frame;
  AVPacket        audio_pkt;       //当前解码音视频的包
  uint8_t         *audio_pkt_data; //当前解码音视频的包的数据
  int             audio_pkt_size;  //当前解码音视频的包的大小
...
int audio_decode_frame(VideoState *is, uint8_t *audio_buf, int buf_size) {
  ...
  for(;;) {
    while(is->audio_pkt_size > 0) { //判断有没有获取到下个包
      int got_frame = 0;
...
//	memcpy(audio_buf, is->audio_frame.data, data_size);
      }
      is->audio_pkt_data += len1;
      is->audio_pkt_size -= len1; // is->audio_pkt_size->while(is->audio_pkt_size > 0)
      if(data_size <= 0) {
    ...
    is->audio_pkt_size = pkt->size; //is->audio_pkt_size->while(is->audio_pkt_size > 0)
  }
}
...
void init_screen(int w, int h){
    if (!screen) {
        ...
        screen = SDL_CreateWindow("sdl", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, w, h, flags);
...
//  SDL_LockMutex(screen_mutex);
//  vp->bmp =  SDL_CreateTexture(renderer, SDL_PIXELFORMAT_ARGB8888, SDL_TEXTUREACCESS_STREAMING, is->video_ctx->width, is->video_ctx->height);
//    SDL_SetTextureBlendMode(vp->bmp, SDL_BLENDMODE_BLEND);
...
//    
//    av_image_fill_arrays(is->video_frame.data, is->video_frame.linesize,out_buffer,
...
//  AVPicture pict;
  ...
  if(vp->bmp) {
//      SDL_LockTexture(vp->bmp, NULL,NULL, NULL);
...
//    pict.linesize[2] = vp->bmp->pitches[1];
     int ret = SDL_UpdateYUVTexture(vp->bmp, &rect, pFrame->data[0], pFrame->linesize[0],
...
//        swr_free(&is->swr_ctx);
        ...
        is->swr_ctx = swr_alloc_set_opts(is->swr_ctx,
                                         out_channel_layout, sample_format, codecCtx->sample_rate,
